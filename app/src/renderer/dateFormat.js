...
module.exports.tomorrow = function(date, formatStyle) {
...
module.exports.thisWeekDays = function(date) {
  ...
  switch (date.toFormat("DDD")){
    case 'Mon':
      dates[0] = date;
      dates[1].setDate(original.getDate() + 1);
      dates[2].setDate(date.getDate() + 2);
      dates[3].setDate(date.getDate() + 3);
      dates[4].setDate(date.getDate() + 4);
      break;
    case 'Tue':
      dates[0].setDate(date.getDate() - 1);
      dates[1] = date;
      dates[2].setDate(date.getDate() + 1);
      dates[3].setDate(date.getDate() + 2);
      dates[4].setDate(date.getDate() + 3);
      break;
    case 'Wed':
      dates[0].setDate(date.getDate() - 2);
      dates[1].setDate(date.getDate() - 1);
      dates[2] = date;
      dates[3].setDate(date.getDate() + 1);
      dates[4].setDate(date.getDate() + 2);
      break;
    case 'Thu':
      dates[0].setDate(date.getDate() - 3);
      dates[1].setDate(date.getDate() - 2);
      dates[2].setDate(date.getDate() - 1);
      dates[3] = date;
      dates[4].setDate(date.getDate() + 1);
      break;
    case 'Fri':
      dates[0].setDate(date.getDate() - 4);
      dates[1].setDate(date.getDate() - 3);
      dates[2].setDate(date.getDate() - 2);
      dates[3].setDate(date.getDate() - 1);
      dates[4] = date;
      break;
  ...
  for (var i=0; i<5; i++) {
    dates[i] = dates[i].toFormat("YYYY-MM-DD")
  }
  return dates;
};
...
module.exports.getStatusDay = function(date) {
  ...
  for (var i = 1; i <= dateNum[today.month-1]; i++){
    var userDate = new Date(today.year, today.month-1, i),
        dayFormat = i;

    if (i< 10) {
      dayFormat = '0' + i;
    }
    if (getHolidays(2016)[today.year + '/' + today.month + '/' + dayFormat]) {
      specialHoliday++;
      if (i <= today.day) {
        today.untilSpecialDay++;
      }
    }
  ...
  today.currentday = today.currentday - today.untilSpecialDay;
  today.businessday = today.businessday - specialHoliday;

  today.format = today.month + '/' +  today.day;
  today.time = date.getTime();

  return today;

}

module.exports.getBusinessDayArray = function(start, end) {
  var startStatus = dateFormat.getStatusDay(start),
      endStatus = dateFormat.getStatusDay(end);

  var num = endStatus.day - startStatus.day,
      array = [],
      weekday;

  for(var i=0; i < num + 1; i++) {
    weekday = dateFormat.getStatusDay(startStatus.year + '-' + startStatus.month + '-' + (Number(startStatus.day)+i)).weekday;
    if (weekday !== 'Sat' && weekday !== 'Sun' && !getHolidays(2016)[startStatus.year + '/' + startStatus.month + '/' + ('0' + (Number(startStatus.day)+i)).slice(-2)]) {
      array.push(i);
    }
  }
  return array;
