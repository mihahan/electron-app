...
module.exports.getStatusDay = function(date) {
  ...
  today.year =　date.toFormat('YYYY');
  today.month = date.toFormat('MM');
  today.day = date.getDate();
  today.weekday = weekDayList[date.getDay()] ;
  today.businessday = 0;
  today.currentday = 0;
  today.untilSpecialDay = 0;

  today.monthFormat = today.month;
  today.dayFormat = today.day;

  if (today.month < 10) {
    today.monthFormat = '0' + today.month;
  }
  ...
  for (var i = 1; i <= dateNum[today.month-1]; i++){
    ...
    if (getHolidays(2016)[today.year + '/' + today.monthFormat + '/' + dayFormat]) {
      specialHoliday++;
      if (i<=today.day) {
...
// console.log(getStatusDay(new Date('2016/11/30')).currentday - getStatusDay(new Date()).currentday);
